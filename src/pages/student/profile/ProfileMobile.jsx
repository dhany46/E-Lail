...
const ProfileMobile = () => {
    ...
    useEffect(() => {
        ...
        const semesterActivities = allActivities.filter(a => {
            const activityDate = new Date(a.rawDate);
            return activityDate >= semesterStart && activityDate <= semesterEnd;
        });

        // Calculate stats
        const totalPoints = semesterActivities.reduce((sum, a) => sum + (a.points || 0), 0);
        const pointsIn = semesterActivities.filter(a => (a.points || 0) > 0).reduce((sum, a) => sum + a.points, 0);
        const pointsOut = semesterActivities.filter(a => (a.points || 0) < 0).reduce((sum, a) => sum + Math.abs(a.points), 0);

        setStats({
            totalPoints,
            totalActivities: semesterActivities.length,
            pointsIn,
            pointsOut,
            semesterLabel,
            academicYearLabel
