...
// Storage helper - abstracts localStorage access for future migration
// Uses sync pattern for backward compatibility
const getAllDailyReportsSync = () => {
    const reports = [];
    const prefix = 'daily_report_';
...
export const loadAllActivities = () => {
    ...
    // Use abstracted storage access
    const reports = getAllDailyReportsSync();

    for (const { date: dateStr, data } of reports) {
        if (!data) continue;
